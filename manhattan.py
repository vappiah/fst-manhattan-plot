...
def mhat(df="dataframe", chr=None, fst=None, color=None, dim=(8,4), r=300, ar=None, fst_sign_line=False,
        ...
        for label, df1 in df.groupby(chr):
            df1.plot(kind='scatter', x='ind', y=fst, color=color_list[i], s=dotsize, alpha=valpha, ax=ax)
        ...
        if fst_sign_line is True:
            ax.axhline(y=fst_thresh, linestyle='--', color='yellow', linewidth=1)
        if markernames is not None:
            marker.geneplot_mhat(df, markeridcol, chr, fst, gwasp, markernames, gfont, gstyle, ax=ax)
        ax.margins(x=0)
        ax.margins(y=0)
        ax.set_xticks(xticks)
        ax.set_ylim([0, max(df[fst] + 1)])
        ...
        ax.set_ylabel(_y, fontsize=axlabelfontsize, fontname=axlabelfontname,loc='bottom',)
